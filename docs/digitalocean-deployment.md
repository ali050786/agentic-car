...
Wherever you manage `blinkwiser.com` DNS (GoDaddy), add:

| Type | Name     | Value            | TTL |
|------|----------|------------------|-----|
| A    | `worker` | `168.144.184.25` | 1/2 Hour (or 600s) |

GoDaddy has no proxy toggle, so there's nothing else to set. Verify from your
...
> **If you ever move DNS to Cloudflare:** set the record to **DNS only** (grey
> cloud), not Proxied — the proxy interferes with Caddy's first cert issuance.

## Step 1 — Get a shell on the droplet

Either the DigitalOcean **Web Console** (droplet page, top right — root shell in
the browser, no key needed), or SSH from your Mac:
...
cd agentic-car
ls docker-compose.yml Caddyfile   # both should list (they're committed to the repo)
```

## Step 6 — (Only if the Compose/Caddy files are missing from the clone)

They're committed to the repo, so Step 5 normally covers this. If `ls` above
didn't find them, create them in the project root:
...
Verify the file landed without printing secrets:

```bash
grep -q '^APPWRITE_API_KEY=' .env && echo "secrets present"; grep '^APP_ORIGIN=' .env
```
...
```bash
docker compose up -d --build      # first build takes 3–8 min on a 1 GB box
docker compose ps                 # both containers Up, worker "healthy"
docker compose logs -f worker     # look for: 🧵 Background worker running on http://localhost:4000
```
...
Check cert issuance on the server if needed:

```bash
docker compose logs caddy | grep -i "certificate obtained"
```

If the cert isn't ready, the usual cause is DNS not yet pointing at the droplet
(Step 0).
...
Both containers use `restart: unless-stopped`, so they survive reboots and
crashes. On restart the worker re-queues anything left `queued` and fails jobs
stuck `running` past 10 min (see `worker/queue.ts`).
